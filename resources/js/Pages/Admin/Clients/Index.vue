            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-400">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-center">Tickets</th><th class="px-5 py-3">Added</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="c in clients.data" :key="c.id" class="hover:bg-slate-50">
                        <td class="px-5 py-3"><Link :href="route('admin.clients.show', c.id)" class="font-medium text-brand-purple hover:underline">{{ c.name }}</Link></td>
                    ...
                    <tr v-if="!clients.data.length"><td colspan="6" class="px-5 py-12 text-center text-slate-400">No clients yet. Add your first client to select them on invoices and tickets.</td></tr>
                </tbody>
            </table>
        </div>
